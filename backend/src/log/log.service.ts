...
export class LogService {
  ...
  sanitizeParams(payload: unknown) {
  ...
  buildOperationMeta(
    ...
    const normalizeCount = () =>
      Array.isArray(body.ids) ? body.ids.length : undefined;
    const qualityStatusMap: Record<string, string> = {
      '2': '确认通过',
      '3': '判定需整改',
    };
    const followUpStatusMap: Record<string, string> = {
      '1': '人工跟进中',
      '2': '已回访',
      '3': '暂不处理',
    };
    const violationStatusMap: Record<string, string> = {
      '1': '已处理',
      '2': '误报',
    };
    const defaultMeta = {
  ...
  async createOperationLog(data: Prisma.OperationLogCreateInput) {
    return this.prisma.operationLog.create({ data });
  }

  async createSystemLog(data: Prisma.SystemLogCreateInput) {
    return this.prisma.systemLog.create({ data });
  }

  async findAllOperationLogs(query: any) {
    const {
      page = 1,
      pageSize = 20,
      username,
  ...
  async findOperationLogsPage(query: any) {
    const {
      page = 1,
      pageSize = 20,
      username,
  ...
  async findAllSystemLogs(query: any) {
    const {
      level,
      module,
      message,
  ...
  async findSystemLogsPage(query: any) {
    const {
      level,
      module,
      message,
  ...
  async findSystemLogsStats(query: any) {
    const {
      dateRange = '7d',
      level,
      module,
    ...
    for (const item of logs) {
      ...
      levelMap.set(
        item.level || 'UNKNOWN',
        (levelMap.get(item.level || 'UNKNOWN') || 0) + 1,
      );
      moduleMap.set(
        item.module || 'UNKNOWN',
        (moduleMap.get(item.module || 'UNKNOWN') || 0) + 1,
      );
      if (item.stack) {
        stackCount += 1;
      }
    ...
    return {
      ...
      levelDistribution: Array.from(levelMap.entries()).map(
  ...
  async findViolationLogs(query: any, user: any) {
    const {
      id,
      page = 1,
      pageSize = 10,
    ...
    const sessionIds = Array.from(
      new Set(alerts.map((item) => item.sessionId).filter(Boolean)),
    );
  ...
  async findViolationStats(query: any, user: any) {
    ...
    const agentMap = new Map<
      string,
      { value: number; userId: string | null }
    >();
    ...
    for (const item of alerts) {
      ...
      statusMap.set(
        this.resolveViolationStatusLabel(item.status),
        (statusMap.get(this.resolveViolationStatusLabel(item.status)) || 0) + 1,
      );
  ...
  async handleViolation(
    id: string,
    body: { status: number; handleRemark?: string },
    user: any,
  ) {
  ...
  async handleViolationsBulk(
    body: { ids: string[]; status: number; handleRemark?: string },
    user: any,
  ) {
  ...
  private resolveViolationStatusLabel(status: number) {
    if (status === 1) {
      return '已处理';
    }
    if (status === 2) {

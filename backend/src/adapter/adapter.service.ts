...
export class AdapterService {
   ...
   */
  async findInterfaces(user: { roles?: string[]; deptId?: string | null }) {
  ...
  async createInterface(payload: AdapterUpsertPayload) {
  ...
  async updateInterface(interfaceId: string, payload: AdapterUpsertPayload) {
    return this.prisma.$transaction(async (tx) => {
  ...
  async updateInterfaceStatus(interfaceId: string, status: number) {
  ...
  private async resolveAutoQualityRuleId(
    deptId?: string | null,
  ): Promise<string | null> {
   ...
   * 接收第三方平台实时 Webhook (全自动入口)
   */
  async processRealtimeMessage(platformCode: string, rawBody: any) {
    const startedAt = Date.now();
    let interfaceId = '';
    let normalizedPayload: Record<string, any> | null = null;
    ...
    try {
      ...
      if (!config) {
        throw new Error(
          `No active adapter interface found for platform ${platformCode}`,
        );
      }

      interfaceId = config.id;
      const normalized = this.transformData(
        rawBody,
        config.mappings,
      ) as NormalizedMessage;
      normalizedPayload = normalized as Record<string, any>;
      const sessionId =
        this.ensureString(normalized.sessionId) || `wh_${Date.now()}`;
      ...
      if (isSessionEnd) {
        this.resolveAutoQualityRuleId(session.deptId)
          .then((ruleId) => {
            if (!ruleId) {
              this.logger.warn(
                `Skip auto quality for session ${session.id}: no active quality rule found`,
              );
              return;
            }
          ...
          .catch((err) =>
            this.logger.error('Auto quality trigger failed', err),
          );
      }
   ...
   * 采集聊天记录的核心逻辑 (支持异构映射)
   */
  async collectChatData(interfaceId: string, options?: { persist?: boolean }) {
    ...
    try {
      ...
      this.logger.error(
        `Error collecting data from interface ${interfaceId}:`,
        error,
      );
      throw error;
    }
  ...
  /**
   * 深度映射引擎：支持 path 访问 (如 'a.b.c') 与格式转换
   */
  private transformData(rawData: any, mappings: any[]) {
    if (!mappings || mappings.length === 0) return rawData;

    const transformSingle = (item: any) => {
      const result: any = {};
      mappings.forEach((mapping: MappingRecord) => {
        const value = this.resolveMappedValue(item, mapping.thirdPartyFields);
        if (value !== undefined) {
          const finalValue = this.applyFormatMapping(
            value,
  ...
  private async replaceMappings(
    tx: Pick<PrismaService, 'dataMapping'>,
    interfaceId: string,
    mappings: AdapterUpsertPayload['mappings'],
  ) {
    ...
    const validMappings = (mappings || []).filter(
      (item) =>
        String(item?.thirdPartyFields || '').trim() &&
  ...
  async setFakeData(interfaceId: string, data: any, scene: string) {
  ...
  async toggleFakeMode(interfaceId: string, enable: boolean) {
    return this.prisma.adapterInterface.update({
      where: { id: interfaceId },
  ...
  async getMonitor(interfaceId: string) {
  ...
  async previewMapping(interfaceId: string, payload: any) {
    const config = await this.prisma.adapterInterface.findUnique({
      where: { id: interfaceId },
      include: { mappings: true, platform: true, department: true },
    });
    ...
    let shopMatch: { id: string; name: string; code: string } | null = null;
    try {
      const shop = await this.resolveShop(
        config.platformId,
        config.deptId,
    ...
    return {
      ...
      preview: {
        ...
        sendTime:
          this.normalizeDate(normalized.sendTime)?.toISOString() || null,
  ...
  private async persistCollectedMessages(config: any, transformed: any) {
    ...
    for (const row of rows) {
      ...
      const result = await this.persistNormalizedMessage(
        config,
        row as NormalizedMessage,
      );
  ...
  private async persistNormalizedMessage(
    config: any,
    normalized: NormalizedMessage,
  ) {
    const sessionId =
      this.ensureString(normalized.sessionId) ||
    ...
    let alertCount = 0;
    if (senderType === 'AGENT' && record.content.trim()) {
      const alerts = await this.keywordService.detectKeywords(
        record.content,
        session.deptId,
        session.id,
        session.userId || '',
  ...
  private resolveMappedValue(item: any, fieldExpression: string) {
    ...
    try {
      ...
      // ignore invalid JSON expression and continue with fallback syntax
    }
    ...
    try {
      ...
      if (parsed.type === 'concat' && Array.isArray(parsed.fields)) {
        return parsed.fields
          .map((field: string) => this.resolveMappedValue(source, field))
          .filter(
  ...
  private async resolveShop(
    ...
    if (!fallbackShop) {
      throw new Error(
        'No shop configuration found for current adapter interface.',
      );
    }
  ...
  private normalizeSenderType(value: any) {
  ...
  private normalizeBoolean(value: any) {
    ...
    return ['true', '1', 'yes', 'y', 'done', 'closed', 'end'].includes(
      normalizedValue,
    );
  }

  private ensureString(value: any) {
  ...
  private async writeAdapterAuditLog(
    level: 'info' | 'error',
    event: string,
  ...
  private pickSamplePayload(payload: any) {
  ...
  private safeJsonStringify(payload: Record<string, unknown>) {
    try {

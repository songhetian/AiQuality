...
export class InsightService {
  ...
  private readonly defaultLossWaitMinutes = 120;
  private readonly minimumQuestionLength = 6;
  ...
  /**
   * 每日凌晨 2:00 自动执行：同步、分析并聚合高频问题 (支持标签发现)
   */
  ...
  async analyzeLoss(sessionId: string) {
  ...
  async getLossAnalysis(query: any, user: any) {
    const {
      id,
      page = 1,
      pageSize = 10,
      ...
      async () => {
        ...
        const list = rawList.map((item) => {
          ...
          return {
            ...
            followUpStatusLabel: this.resolveFollowUpStatusLabel(
  ...
  async getLossStats(query: any, user: any) {
    ...
    const trendMap = new Map<string, { lost: number; retained: number }>();
    ...
    for (const item of records) {
      ...
      shopMap.set(
        shopName,
        (shopMap.get(shopName) || 0) + (item.isLost ? 1 : 0),
      );

      const followUpName = this.resolveFollowUpStatusLabel(
        (item as any).followUpStatus,
      );
  ...
  async updateLossFollowUp(id: string, body: any, user: any) {
  ...
  async batchUpdateLossFollowUp(body: any, user: any) {
  ...
  async getHighFreqQuestions(query: any) {
    const {
      userId,
      productId,
      date,
  ...
  private async aggregateHighFreqQuestions(statDate: Date) {
  ...
  private normalizeQuestionContent(content?: string | null) {
  ...
  private inferQuestionTagName(content: string) {
  ...
  private async ensureQuestionTag(name: string) {
  ...
  private async ensureTagAuditHint(tagName: string, sampleContent: string) {
  ...
  async getLossRule(query: any, user: any) {
  ...
  async saveLossRule(body: any, user: any) {
    ...
    const waitMinutes = Math.max(
      5,
      Number(body?.waitMinutes || this.defaultLossWaitMinutes),
    );
  ...
  private async refreshLossAnalysis(rangeStart: Date, user: any) {
  ...
  private async upsertLossAnalysis(session: any) {
    ...
    const customerMessages = records.filter(
      (item: any) => item.senderType === 'CUSTOMER',
    ).length;
    const agentMessages = records.filter(
      (item: any) => item.senderType === 'AGENT',
    ).length;
    const latestTime =
      lastRecord?.sendTime || session.updateTime || session.createTime;
    const inactiveMinutes = Math.max(
      0,
      Math.floor((Date.now() - new Date(latestTime).getTime()) / 60000),
    );
    const sessionDurationMinutes =
      firstRecord && lastRecord
        ? Math.max(
            1,
            Math.floor(
              (new Date(lastRecord.sendTime).getTime() -
  ...
  private resolveRangeStart(dateRange: string) {
    const now = new Date();
  ...
  private extractWaitMinutes(reason?: string | null) {
    const matched = String(reason || '').match(/(\d+)\s*分钟/);
    return matched ? Number(matched[1]) : null;
  }

  private summarizeLossReason(reason?: string | null) {
  ...
  private resolveFollowUpStatusLabel(status?: number | null) {
    if (status === 1) {
      return '人工跟进中';
    }
    if (status === 2) {
  ...
  private resolveQualityStatusLabel(status?: number | null) {
    if (status === 0) {
      return '质检中';
    }
    if (status === 1) {
  ...
  private resolveLossConfidence(input: {
    isLost: boolean;
    inactiveMinutes: number;
    waitMinutes: number;
    customerMessages: number;
